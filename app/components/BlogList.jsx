...
function BlogList({ posts, isLoading }) {
  return (
    <div>
      {/* <div>{isLoading ? <p>true</p> : <p>false</p>}</div> */}
                  ...
                  <div className="absolute bottom-0 w-full bg-opacity-20 bg-black backdrop-blur-lg rounded drop-shadow-lg text-white p-3 flex justify-between">
                    ...
                    <div className="flex flex-col md:flex-row gap-y-2 md:gap-x-2 items-center">
                      {post.categories.map((category) => (
                        <div key={category._id} className="bg-yellow-400 text-center text-black px-3 py-1 rounded-full text-sm font-semibold dark:bg-purple-400">
                          <p>{category.title}</p>
                        </div>
                      ))}
                    </div>
